...
;; Copyright (C) 2001-2009 Alexander Baltatzis, Johan Dykstrom
...
;;  0.97  2009-11-06  Added support for Java enums. Binding case-fold-search
;;                    with let instead of setting it with setq. Fixed
...
(defconst jtags-version "0.97-b3"
...
qualified class names."
  ...
  (let ((definition (jtags-lookup-identifier class nil package-list))
        (regexp (concat "\\(class\\|interface\\)\\W+"
                        class
                        "\\W+.*extends\\W+"
...
The optional argument BOUND bounds the search; it is a buffer position."
  (let (result)
    ;; Find class name
    (while (re-search-forward "\\(class\\|enum\\|interface\\) \\([A-Za-z0-9_]+\\)" bound t)

      ;; If we found a match, save class name and find line of declaration
      (let ((class (match-string 2))
            line)
        (jtags-message "Found `%s' on line %s in tags file" class (jtags-get-line))
    ...
    (save-excursion
      (jtags-save-buffer-state ()
        ...
        (while (and (null start-pos) (re-search-backward start-regexp bound t))
          (jtags-message "Match `%s'" (match-string 0))
